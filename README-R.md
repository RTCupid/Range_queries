...
- Полный контроль над структурой данных — дерево реализовано без использования std::set или других стандартных контейнеров, что позволило гибко расширить функциональность, включая хранение размера поддерева и возможность подробного графического дампа.
...
- вспомогательное поле — размер поддерева (size_), используемое для диапазонных запросов;
- служебные методы для обновления размера поддерева.

#### tree.hpp

Определяет основной класс RB_tree, включающий:
- операции вставки (перекрашивание, левый/правый поворот);
- операции поиска;
...
| `TIMING_RUN`   | bool: `ON`/`OFF`    | `OFF`  | Включает вывод времени выполнения (измерение производительности) | Активирует вывод или логику, связанную со временем, под `#ifdef TIMING_RUN`|
